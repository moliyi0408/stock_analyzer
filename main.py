...
def print_analysis(stock_id, df, result):
    ...
    print(f"主升段候選：{safe_get('is_primary_uptrend_candidate', False)}")
    print(f"候選理由：{safe_get('primary_uptrend_candidate_reasons', [])}")
    print(f"候選訊號：{safe_get('primary_uptrend_candidate_signals', {})}")
